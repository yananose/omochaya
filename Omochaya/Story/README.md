...
極限のパフォーマンスを求める場合や、タスクの「一時停止（ポーズ）」を実装したい場合は、Bootに指定するコンポーネントに `Story.ITaskOwner` インターフェースを実装してください。あるいは用意されている `Story.TaskBehaviour` を継承するとコンポーネントの非アクティブ時に一時停止させつつ偽装nullチェックを回避できます。
```csharp
