...
* **安全なライフサイクル管理**: `MonoBehaviour` などのコンポーネントをオーナーとして紐付け、オブジェクト破棄時の安全なタスクキャンセルを保証。
...
public class Actor : MonoBehaviour 
{
    ...
    async Story.Task ActionSequence()
    {
        ...
        // 7. 【Update層】終了
        Debug.Log("Startで指定したコンポーネントが破棄されても処理中のタスクは await まで実行されます");
        await Story.Yield;
...
極限のパフォーマンスを求める場合や、タスクの「一時停止（ポーズ）」を実装したい場合は、Startに指定するコンポーネントに `Story.ITaskOwner` インターフェースを実装してください。あるいは用意されている `Story.TaskBehaviour` を継承するとコンポーネントの非アクティブ時に一時停止させつつ偽装nullチェックを回避できます。
```csharp
// TaskBehaviour を継承（または ITaskOwner を実装）することで、
